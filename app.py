...
class cargo():    
    ...
    def main(page: ft.Page):
        ...
        def list():
            ...
            for row in cursor:
                lv.controls.append(ft.Container(ft.Text(f"{row[0]}, {row[1]},{row[2]},{row[3]}"),border=ft.border.all(2,'black')))
            page.add(ft.Row(controls=[ft.TextField(label="Вес"),ft.TextField(label="Страна происхождения")]),
                     ft.Row(controls=[ft.TextField(label="Дата"),ft.ElevatedButton(text="Добавить в базу",on_click=add_car)]),
                     ft.Container(content=lv,width=550,height=300,border=ft.border.all(3,'yellow'),border_radius=8))
            conn.close()
